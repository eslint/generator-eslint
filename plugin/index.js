...
export default class extends Generator {
	async prompting() {
		...
		this.answers.pluginId = this.answers.pluginId.replace(
			"eslint-plugin-",
			"",
		);
	}
	...
	writing() {
		...
		if (this.answers.hasRules) {
			mkdirSync(this.destinationPath("lib", "rules"), {
				recursive: true,
			});
			mkdirSync(this.destinationPath("tests", "lib", "rules"), {
				recursive: true,
			});
		}

		if (this.answers.hasProcessors) {
			mkdirSync(this.destinationPath("lib", "processors"), {
				recursive: true,
			});
			mkdirSync(this.destinationPath("tests", "lib", "processors"), {
				recursive: true,
			});
		}
	}
